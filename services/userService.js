...
exports.createUser = async (userBody) => {
  ...
  if (user) throw new AppError("Username Already Exist!", 405);

  user = await User.findOne({ where: { email } });
  if (user) throw new AppError("Email Already Exist!", 405);
...
exports.updateUser = async (req) => {
  ...
  if (!userUpdated[0]) throw new AppError(`User did not update`, 404);
...
exports.deleteUser = async (req) => {
  ...
  if (!validPass) throw new AppError("Invalid Credential", 404);
...
exports.loginUser = async (req, res) => {
  ...
  if (user == null) throw new AppError("Invalid Credential", 404);
  const validPass = await bcrypt.compare(password, user.password);
  if (!validPass) throw new AppError("Invalid Credential", 404);
...
exports.logoutUser = async (req) => {
  ...
  if (!refresh_token) throw new AppError("Bad Request", 400);
  try {
    ...
    await User.update(
      ...
      { returning: true, where: { username: decoded.user } }
    );
  } catch (err) {
